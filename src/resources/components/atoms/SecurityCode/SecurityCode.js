    ...
    */
    ...
    const generateSerialInput = () => {
        ...
        for (let i = 0; i < length; i += 1) {
            serialInputList[i] = (React.createElement(Col, { key: `serialInput_${i}` },
                React.createElement(SerialInput, { ref: e => {
                        serialRef.current[i] = e;
                    }, maxLength: 1, onClick: handleFocusNext, onKeyUp: handleSyncValue, onChange: () => handleMoveToNextInput(i), onKeyDown: e => handleBackInput(e, i), placeholder: " ", type: "text" })));
        }
        return serialInputList;
    };
    return (React.createElement(SecurityCodeRoot, { className: className, style: style, length: length },
        generateSerialInput(),
        React.createElement("input", { ref: e => {
                forwardRef(e);
                inputRef.current = e;
...
const SecurityCodeRoot = styled.div `
    width: ${(props) => `calc(${props.length} * ${px2vw(50)})`};
    max-width: 100%;
    display: flex;
    flex-direction: row;
    flex-wrap: wrap;
    ...
    ${media.lg `
        width: ${(props) => `calc(${props.length} * 50px)`};
    `}
`;
